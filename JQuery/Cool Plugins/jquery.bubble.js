    ...
    $.fn.bubble = function (options) {
        ...
        let canvas,
            ...
            funcImgLoad = (callback) => {
                ...
                if (l === 0) callback(eleImg);
                let count = 0;
                for (var i = 0; i < l; ++i) {
                    eleImg[i] = $("<img>");
                    eleImg[i].on("load error", () => {
                        count++;
                        if (l === count) callback(eleImg);
                    });
                    eleImg[i].attr("src", configs.img[i]);
                }
            },
            funcPutCanvas = () => {
                el.append(
			...
			"
                ...
                const eleCanvas = $(
                    selectorRule.class + configs.namespace + "canvas",
                    el
                );
            ...
            funcEditConfigs = () => {
                if (configs.bubbles === false)
                    configs.bubbles = Math.floor(
                        configs.granularity * (canvasWidth + canvasHeight)
                    );
                ...
                canvas.clearRect(0, 0, canvasWidth, canvasHeight),
                    bubblesConfigs.forEach((object) => {
                        canvas.beginPath(),
                            ...
                            canvas.fill(),
                            (object.x +=
                                Math.cos(object.angle) * object.velocity),
                            (object.y +=
                                Math.sin(object.angle) * object.velocity),
                            object.x - object.radius > canvasWidth &&
                                (object.x = -object.radius),
                            object.x + object.radius < 0 &&
                                (object.x = canvasWidth + object.radius),
                            object.y - object.radius > canvasHeight &&
                                (object.y = -object.radius),
                            object.y + object.radius < 0 &&
                                (object.y = canvasHeight + object.radius);
                    });
